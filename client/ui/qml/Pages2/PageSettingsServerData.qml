...
PageType {
    ...
    QtObject {
        ...
        property bool isVisible: root.isServerWithWriteAccess
        readonly property string title: qsTr("Clear server from DefaultVPN software")
        readonly property string description: ""
        readonly property var tColor: AmneziaStyle.color.vibrantRed
        readonly property var clickedHandler: function() {
            var headerText = qsTr("Do you want to clear server from DefaultVPN software?")
            ...
            var yesButtonFunction = function() {
                if (ServersModel.isDefaultServerCurrentlyProcessed() && ConnectionController.isConnected) {
                    PageController.showNotificationMessage(qsTr("Cannot clear server from DefaultVPN software during active connection"))
                } else {
